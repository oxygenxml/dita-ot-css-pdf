    ...
    <xsl:function name="oxy:is-chapter" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$doc/*[contains(@class, ' bookmap/bookmap ')]">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="contains($toc-item/@class,' map/topicref ') and $toc-item/parent::opentopic:map">
                        <!-- Consider the first level children, like appendix or index as chapters. 
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="$toc-item/parent::opentopic:map  and contains($toc-item/@class, ' map/topicref ') and not(contains($toc-item/@class, ' glossref-d/glossref '))">
                        <xsl:value-of select="true()"/>
                    </xsl:when>        
                    <xsl:otherwise>
                        <xsl:value-of select="false()"/>
